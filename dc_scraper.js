...
// Uses Claude Vision API instead of Tesseract for accurate extraction
...
// ── Claude Vision: extract all ads from page image ─────────────────────────
...
{
  "title": "first line / heading of the ad",
  "description": "complete ad text as printed",
  "phone": "10-digit number or empty string",
  "price": "price like ₹45 L or ₹2.5 Cr or Not mentioned",
  "location": "area in Hyderabad or empty string",
...
Rules:
- Read ALL 5 columns top to bottom
- Include every ad even if phone number is missing
- For phone: extract 10-digit mobile numbers starting with 6-9
- For category Property: sub_category = For Sale / For Rent / PG / Hostel
- For category Jobs: sub_category = Full-time / Part-time
- For category Automotive: sub_category = Used vehicle
- For category Matrimonial: sub_category = Bride Sought / Groom Sought / Alliance
- section headers like MULTIPLE VACANCIES, FOR SALE PROPERTY, LEASE etc. indicate category for ads below them`;
...
async function extractAdsWithVision(imagePath) {
  ...
  console.log(`[DC] Sending ${sizeKB}KB image to Claude Vision…`);
  ...
  console.log(`[DC] Claude response (${raw.length} chars): ${raw.slice(0,200)}`);
  ...
  try {
    ...
    throw new Error(`JSON parse failed: ${e.message} — raw: ${clean.slice(0,300)}`);
  }
}
...
async function getPageImageFromViewer(page, pgNum, outPath) {
  ...
  const pngBase64 = await page.evaluate(() => {
    const selectors=['#imgPage','#pageImage','#mainImage',
      'img[id*="imgPage"]','img[id*="PageImage"]','img[id*="mainImg"]'];
    let img=null;
    for(const s of selectors){const el=document.querySelector(s);if(el&&el.naturalWidth>400){img=el;break;}}
    if(!img) img=[...document.querySelectorAll('img')]
  ...
  if(pngBase64.startsWith('__SRC__')){
    ...
    console.log(`[DC] Canvas export: ${(fs.statSync(outPath).size/1024).toFixed(0)}KB`);
  }
  return true;
}

// ── Build ads from Claude JSON output ─────────────────────────────────────
function buildAds(rawAds, publishDate) {
  ...
  function guessLocation(desc, locHint){
    if(locHint&&locHint.length>2)return locHint.includes('Hyderabad')?locHint:`${locHint}, Hyderabad`;
  ...
  return rawAds
    ...
    .map(a=>({
      ...
      location:       guessLocation(a.description||'', a.location||''),
    ...
    .filter(a=>a.title.length>2||a.description.length>10);
}
...
async function scrapeDate(page, targetDate) {
  ...
  for(const pgNum of [CLASSIFIEDS_PAGE, CLASSIFIEDS_PAGE+1]){
    ...
    // Try canvas export from viewer first
    ...
    if(!gotImage||!fs.existsSync(imgPath)||fs.statSync(imgPath).size<5000){
      console.log(`[DC] Trying direct URL download for page ${pgNum}…`);
      const urls=[
      ...
      for(const url of urls){
        try{
          const p=imgPath.replace('.png','.jpg');
          await downloadFile(url,p);
          const sz=fs.statSync(p).size;
          if(sz>5000){console.log(`[DC] Downloaded ${(sz/1024).toFixed(0)}KB`);gotImage=true;break;}
    ...
    // Find the actual image file (might be .jpg or .png)
    const actualPath = fs.existsSync(imgPath) ? imgPath : imgPath.replace('.png','.jpg');
    const sz = fs.statSync(actualPath).size;
    console.log(`[DC] Image ready: ${(sz/1024).toFixed(0)}KB — sending to Claude Vision`);

    try {
      // Claude Vision extracts all ads — no Tesseract, no column splitting needed
      const rawAds = await extractAdsWithVision(actualPath);
      console.log(`[DC] Claude extracted ${rawAds.length} raw ads from page ${pgNum}`);

      const ads = buildAds(rawAds, targetDate);
      console.log(`[DC] Page ${pgNum}: ${ads.length} valid ads`);
      allAds.push(...ads);

      // If page 8 had good results, skip page 9
      ...
      console.error(`[DC] Vision extraction failed for page ${pgNum}: ${e.message}`);
    }
